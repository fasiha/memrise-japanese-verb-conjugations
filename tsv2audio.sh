...
cat course.tsv | grep -v "base\tbase kana" | sed '/^$/d' | while read line; do
  # echo $line
  kana=$(echo "$line" | cut -f2)
  kanji=$(echo "$line" | cut -f5)
  for voice in Mizuki Takumi; do
    if [ ! -f "$voice/$kana.mp3" ]; then
      echo aws polly synthesize-speech --output-format mp3 --voice-id $voice --text "$kanji" "$voice/$kana.mp3"
    fi
  done
done
